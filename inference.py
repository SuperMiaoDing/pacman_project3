    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            new = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            for position, probability in new.items():
                allPossible[position] = allPossible[position] + probability * self.beliefs[pos]
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for position in self.particles:
            newParticles.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, position))))
    ...
    def observeState(self, gameState):
        ...
        A correct implementation will handle two special cases:
          ...
          2) When all particles receive 0 weight, they should be recreated from
             ...
             will involve changing each p if a ghost has been eaten.

        self.getParticleWithGhostInJail is a helper method to edit a specific
        p. Since we store particles as tuples, they must be converted to
        ...
        for p in self.particles:
            ...
            while i < self.numGhosts:
                if noisyDistances[i] == None:
                    ...
                    possibility *= emissionModels[i][util.manhattanDistance(p[i], pacmanPosition)]
                i += 1
            allPossible[p] += possibility
        if not any(allPossible.values()):
            self.initializeParticles()
            for p in self.particles:
                i = 0
